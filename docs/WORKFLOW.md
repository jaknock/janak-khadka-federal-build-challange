...
sequenceDiagram
    ...
    loop Up to eight concurrent analyses
        ...
        Rules-->>API: Per-field findings + Match / Needs review / Mismatch
    end
...
- The mock inbox runs up to eight records concurrently for demo speed. A production
  high-volume workflow would require persistent jobs, rate-limit-aware retries, and
  monitoring.
